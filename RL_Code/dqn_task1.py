...
# Lab5: Value-based RL - Task 1: Vanilla DQN on CartPole-v1
...
class DQN(nn.Module):
    def __init__(self, num_actions):
        ...
        # Fully-connected network for CartPole 4-dim state
...
class AtariPreprocessor:
    """Not used for Task 1 (CartPole), kept for template consistency"""
...
class DQNAgent:
    ...
    def run(self, episodes=500):
        for ep in range(episodes):
            ...
            if self.epsilon > self.epsilon_min and len(self.memory) >= self.replay_start_size:
            ...
            # Evaluate and save best model every 20 episodes
            if ep % 20 == 0:
                ...
                if eval_reward > self.best_reward:
                    ...
                    model_path = os.path.join(
                        self.save_dir, f"LAB5_{self.student_id}_task1.pt")
                    torch.save(self.q_net.state_dict(), model_path)
                    print(f"Saved new best model → {model_path} (reward={eval_reward:.1f})")
    ...
    def train(self):
        ...
        with torch.no_grad():
            # Vanilla DQN: target = r + gamma * max_a' Q(s', a'; theta_bar)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir",               type=str,   default="./results_task1")
